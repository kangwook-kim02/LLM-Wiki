...
트리거 조건은 각 스킬 파일의 `description` 필드에 정의되어 있다.
사용자 메시지가 들어오면 스킬 파일의 `description`을 읽고 해당 스킬을 실행한다.

| 스킬 | 스킬 파일 |
|------|----------|
| **ingest** | `.claude/skills/ingest.md` |
| **query** | `.claude/skills/query.md` |

### 커맨드 (슬래시 트리거)

| 커맨드 | 실행 방법 | 커맨드 파일 |
|--------|----------|------------|
| **lint** | `/lint` 입력 | `.claude/commands/lint.md` |
