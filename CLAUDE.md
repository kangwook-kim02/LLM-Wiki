...
> 절차 상세 내용은 각 스킬 파일을 참조한다.
> 사용자 메시지가 들어오면 아래 트리거 표에서 해당 스킬을 찾아 실행한다.

| 스킬 | 트리거 패턴 | 스킬 파일 |
|------|-----------|----------|
| **ingest** | 파일명 + "위키", "만들어", "인제스트", "추가", "업로드" 중 하나 | `.claude/skills/ingest.md` |
| **lint** | "lint", "점검", "검사", "건강", "고아 페이지", "모순" 중 하나 | `.claude/skills/lint.md` |
| **query** | 위 두 트리거에 해당하지 않는 모든 질문 | `.claude/skills/query.md` |

### 트리거 예시

```
# ingest 트리거
"10. New Topic.pdf 업로드 했어, 위키 만들어줘!"
"paper.pdf 인제스트해줘"
"새 자료 추가했어 — article.md 위키에 넣어줘"
